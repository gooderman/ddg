...
int App::run(void)
{
	...
	string prjdir;
	Wchar_tToString(prjdir,exeFullPath);
	int pos = prjdir.rfind("\\proj.win32\\Debug");
	if(pos==-1){
		pos = prjdir.rfind("\\");
	}
	if(pos!=-1){
		prjdir=prjdir.substr(0,pos);
		m_project.setProjectDir(prjdir);
		m_project.setFrameSize(CCSize(320, 480));
	}
	//====================================================================
